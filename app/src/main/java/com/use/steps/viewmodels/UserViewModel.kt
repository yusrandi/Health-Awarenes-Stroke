...
class UserViewModel : ViewModel() {
    ...
    fun deleteUserById(id: String) {
        ...
        api.deleteUserById(id).enqueue(object : Callback<WrappedResponse<User>> {
            override fun onFailure(call: Call<WrappedResponse<User>>, t: Throwable) {
                Log.d("UserViewModel", "onFailure ${t.message}")
                state.value = UserState.Error(t.message)
            }
            ...
            override fun onResponse(call: Call<WrappedResponse<User>>, response: Response<WrappedResponse<User>>) {
                if (response.isSuccessful) {
                    ...
                    if (body.responsecode.equals("1")) {
                        state.value = UserState.SuccessDeleteAccount(body.responsemsg)
                    } else {
                        state.value = UserState.Error(body.responsemsg.toString())
                    }
                } else {
...
sealed class UserState {
    ...
    data class SuccessDeleteAccount(var message: String?) : UserState()
